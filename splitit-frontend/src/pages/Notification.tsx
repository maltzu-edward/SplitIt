      ...
      <div className="flex-1 overflow-y-auto px-4 pb-20 pt-4">
        ...
        {!loading && notifications.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full mt-[-40px]">
            ...
            <p className="text-base font-bold text-gray-800 dark:text-white">No new notifications</p>
        ...
        {!loading && notifications.length > 0 && (
          <div className="space-y-3">
            {notifications.map((notification: any) => (
              <div key={notification.id} className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-gray-900 dark:text-white">{notification.message}</p>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">{notification.user?.email ?? ""}</p>
                  </div>
                ...
                {notification.type === "FRIEND_REQUEST" && (
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      className="flex-1 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 py-2.5 text-sm font-bold text-green-600 cursor-pointer hover:bg-green-100 dark:hover:bg-green-900/40 transition-colors"
                    >
                      Accept
                    </button>
                    <button
                      onClick={() => handleRespond(notification.id, "DECLINED")}
                      className="flex-1 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 py-2.5 text-sm font-bold text-red-500 cursor-pointer hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
                    >
                      Decline
                    </button>
                  </div>
